...
class BookListNotifier extends EntityListNotifier<Book, BookQuery> {
  ...
  Future<void> setGenre(int? id) {
    return applyQuery(query.copyWith(genreId: id, page: 1));
  }

  Future<void> setPublisher(int? id) {
    return applyQuery(query.copyWith(publisherId: id, page: 1));
  }

  Future<void> setAuthor(int? id) {
    return applyQuery(query.copyWith(authorId: id, page: 1));
  }

  Future<void> setYearFrom(int? year) {
    return applyQuery(query.copyWith(yearFrom: year, page: 1));
  }

  Future<void> setYearTo(int? year) {
    return applyQuery(query.copyWith(yearTo: year, page: 1));
  }

  Future<void> setAvailable(bool? value) {
    return applyQuery(query.copyWith(available: value, page: 1));
  }

  Future<void> resetFilters() {
    return applyQuery(BookQuery(size: query.size));
  }
  ...
  Future<bool> isIsbnFree(String isbn, {int? exceptId}) async {
    final result = await _repository.find(
      const BookQuery(page: 1, size: 1000, includeDeleted: true),
    );
    ...
    return !result.items.any(
      (book) =>
          book.isbn.trim().toLowerCase() == normalized && book.id != exceptId,
    );
  }
  ...
  String urlFor(BookQuery value) {
    final params = <String, String>{
      ...
      if (value.genreId != null) 'genreId': '${value.genreId}',
      if (value.publisherId != null) 'publisherId': '${value.publisherId}',
      if (value.authorId != null) 'authorId': '${value.authorId}',
      if (value.yearFrom != null) 'yearFrom': '${value.yearFrom}',
      if (value.yearTo != null) 'yearTo': '${value.yearTo}',
      if (value.available != null) 'available': '${value.available}',
    ...
    return Uri(path: '/books', queryParameters: params).toString();
  }
}
